...
@SerialVersionUID(1L)
class SeparatingFeaturizerKeepFirst(val featureIndexer: Indexer[String], 
  ...
  def getIndex(feature: String, addToFeaturizer: Boolean): Int = {
    if (!addToFeaturizer) {
      if (!featureIndexer.contains(feature)) {
          val idx = featureIndexer.getIndex(SeparatingFeaturizerKeepFirst.UnkFeatName);
          require(idx == 0);
          idx;
        } else {
  ...
  def featurizeIndexStandard(docGraph: DocumentGraph, currMentIdx: Int, antecedentIdx: Int, addToFeaturizer: Boolean): Seq[Int] = {
    ...
    // only difference is we keep features on first thing
...
object SeparatingFeaturizerKeepFirst {
