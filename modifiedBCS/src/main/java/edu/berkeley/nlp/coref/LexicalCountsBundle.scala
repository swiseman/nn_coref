...
object LexicalCountsBundle {
  ...
  def countLexicalItems(trainDocs: Seq[CorefDoc], cutoff: Int) = {
    ...
    if (Driver.definitenessSubset != "") {
      val entriesToKeep = (if (Driver.definitenessSubset.contains("dets1")) dets1 else Set[String]()) ++
                          (if (Driver.definitenessSubset.contains("dets2")) dets2 else Set[String]()) ++
                          (if (Driver.definitenessSubset.contains("dems")) dems else Set[String]()) ++
                          (if (Driver.definitenessSubset.contains("poss")) poss else Set[String]()) ++
                          (if (Driver.definitenessSubset.contains("mods")) mods else Set[String]()) ++
                          (if (Driver.definitenessSubset.contains("nats")) nats else Set[String]()) ++
                          (if (Driver.definitenessSubset.contains("roles")) roles else Set[String]());
      val presentKeys = new HashSet[String]() ++ firstWordCounts.keySet().asScala;
      for (word <- presentKeys) {
        if (!entriesToKeep.contains(word)) {
          firstWordCounts.removeKey(word);
        }
      }
    }
