...
//      featStrs += ("PrevWordNextWord=" + pw + "-" + fw);
  ...
  def trainSentenceSplitter {
    ...
    val (trainExamples, testExamples) = if (SentenceSplitterTokenizerDriver.trainFromConll) {
      (readExamplesFromConll(CorefSystem.loadRawConllDocs(SentenceSplitterTokenizerDriver.conllTrainPath, SentenceSplitterTokenizerDriver.conllTrainSize, false)),
       readExamplesFromConll(CorefSystem.loadRawConllDocs(SentenceSplitterTokenizerDriver.conllTestPath, SentenceSplitterTokenizerDriver.conllTestSize, false)));
    } else {
