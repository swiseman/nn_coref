...
cmd:option('-pwFullSerFile', 'models/basicp-0.500000-1.200000-1.000000.model-full_g1_PT-pw', 'Path to saved pairwise network portion of trained model')
cmd:option('-anaFullSerFile', 'models/basicp-0.500000-1.200000-1.000000.model-full_g1_PT-na', 'Path to saved anaphoricity portion of trained model')
cmd:text()
...
function main()
    if not opts.loadAndPredict then -- if training, get train data
       ...
       train(pwTrData,anaTrData,trClusts,pwDevData,anaDevData,opts.hiddenPairwise,
            opts.hiddenUnary,opts.fl,opts.fn,opts.wl,opts.eta1,opts.eta2,
       ...
       local anaDevData = SpDMData.loadFromH5(opts.anaDevFeatPrefix)
       print("read anaph dev data")
      local fm = FullG1Model.load(opts.pwFullSerFile, opts.anaFullSerFile)
