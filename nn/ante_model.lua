...
cmd:text()
cmd:text('Options')
cmd:option('-hiddenPairwise', 700, 'Hidden layer size')
...
cmd:option('-nEpochs', 3, 'Number of epochs to train')
cmd:option('-save', false, 'Save best model')
cmd:option('-savePrefix', '', 'Prefixes saved model with this')
...
function main()
    local pwTrData = SpDMPWData.loadFromH5(options.pwTrFeatPrefix)
    print("read pw train data")
    print("max pw feature is: " .. pwTrData.maxFeat)
    local pwDevData = SpDMPWData.loadFromH5(options.pwDevFeatPrefix)
    print("read pw dev data")
    local trClusts = getOPCs(options.trainClustFile,pwTrData)
    print("read train clusters")
    local devClusts = getOPCs(options.devClustFile,pwDevData)
    print("read dev clusters")    
   
    local anteModel = AnteModel.make(pwTrData.maxFeat, options.hiddenPairwise)
