...
cmd:option('-hidden_outer', 128, 'Hidden layer.')
...
cmd:option('-pwFullSerFi', 'models/basicp_128-0.500000-1.200000-1.000000.model-full_g2_PT-pw', 'serialized pairwise network')
cmd:option('-anaFullSerFi', 'models/basicp_128-0.500000-1.200000-1.000000.model-full_g2_PT-na', 'serialized anaphoricity network')
cmd:text()
...
function main()
    if not opts.load_and_predict then -- if training, get train data
       ...
       train(pwTrData,anaTrData,trClusts,pwDevData,anaDevData,opts.hidden_pairwise,
            opts.hidden_unary,opts.hidden_outer,opts.fl,opts.fn,opts.wl,opts.eta1,opts.eta2,
       ...
       local anaDevData = SpDMData.loadFromH5(opts.anaDevFeatPfx)
       print("read anaph dev data")
       local fm = FullG2Model.load(opts.pwFullSerFi, opts.anaFullSerFi)
